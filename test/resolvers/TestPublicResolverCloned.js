...
contract('PublicResolver (Cloned)', function (accounts) {
  ...
  beforeEach(async () => {
    ...
    // console.log(diamondResolver.address, await diamondResolver.getFallbackAddress())

    diamondResolver = await PublicResolver.at(newResolverAddress)
    // console.log(diamondResolver.address, await diamondResolver.getFallbackAddress())
    auth = await RegistryWhitelistAuthFacet.at(newResolverAddress)
    resolver = await PublicResolverFacet.at(newResolverAddress)
  ...
  describe('fallback function', async () => {
    // Ignore because it is more danger to override diamond fallback logic
  ...
  describe('recordVersion', async () => {
    it('permits clearing records', async () => {
      var tx = await diamondResolver.clearRecords(node, { from: accounts[0] })
  ...
  describe('addr', async () => {
    ...
    it('resets record on version change', async () => {
      await basicSetAddr()
      await diamondResolver.clearRecords(node)
  ...
  describe('authorisations', async () => {
    it('permits authorisations to be set', async () => {
      await diamondResolver.setApprovalForAll(accounts[1], true, {
        from: accounts[0],
      })
      assert.equal(
        await diamondResolver.isApprovedForAll(accounts[0], accounts[1]),
        true,
      )
    })
    ...
    it('permits authorised users to make changes', async () => {
      ...
      assert.equal(
        await diamondResolver.isApprovedForAll(await ens.owner(node), accounts[1]),
        true,
      )
      await resolver.methods['setAddr(bytes32,address)'](node, accounts[1], {
        from: accounts[1],
    ...
    it('permits authorisations to be cleared', async () => {
      await diamondResolver.setApprovalForAll(accounts[1], false, {
        from: accounts[0],
      })
      await exceptions.expectFailure(
        resolver.methods['setAddr(bytes32,address)'](node, accounts[0], {
    ...
    it('permits non-owners to set authorisations', async () => {
      await diamondResolver.setApprovalForAll(accounts[2], true, {
        from: accounts[1],
      })
    ...
    it('emits an ApprovalForAll log', async () => {
      ...
      var tx = await diamondResolver.setApprovalForAll(operator, true, {
        from: owner,
      })
      assert.equal(tx.logs.length, 1)
    ...
    it('reverts if attempting to approve self as an operator', async () => {
      await expect(
        diamondResolver.setApprovalForAll(accounts[1], true, { from: accounts[1] }),
  ...
  describe('token approvals', async () => {
    it('permits delegate to be approved', async () => {
      await diamondResolver.approve(node, accounts[1], true, {
        from: accounts[0],
      })
      assert.equal(
        await diamondResolver.isApprovedFor(accounts[0], node, accounts[1]),
        true,
      )
    })
    ...
    it('permits delegated users to make changes', async () => {
      ...
      assert.equal(
        await diamondResolver.isApprovedFor(await ens.owner(node), node, accounts[1]),
        true,
      )
      await resolver.methods['setAddr(bytes32,address)'](node, accounts[1], {
        from: accounts[1],
    ...
    it('permits delegations to be cleared', async () => {
      await diamondResolver.approve(node, accounts[1], false, {
        from: accounts[0],
      })
      await exceptions.expectFailure(
        resolver.methods['setAddr(bytes32,address)'](node, accounts[0], {
    ...
    it('permits non-owners to set delegations', async () => {
      await diamondResolver.approve(node, accounts[2], true, {
        from: accounts[1],
      })
    ...
    it('emits a Approved log', async () => {
      ...
      var tx = await diamondResolver.approve(node, delegate, true, {
        from: owner,
      })
      assert.equal(tx.logs.length, 1)
    ...
    it('reverts if attempting to delegate self as an delegate', async () => {
      await expect(
        diamondResolver.approve(node, accounts[1], true, { from: accounts[1] }),
  ...
  describe('multicall', async () => {
    it('allows setting multiple fields', async () => {
      ...
      var textSet = resolver.contract.methods
        ...
        .encodeABI()
      var tx = await diamondResolver.multicall([addrSet, textSet], {
        from: accounts[0],
      })
    ...
    it('allows reading multiple fields', async () => {
      ...
      var results = await diamondResolver.multicall.call([
